...
def export_raw_data(params: dict, df: pd.DataFrame):
    ...
    df.to_csv(
        f'./data/Fort {params["Subject"]} - fullDF - {datetime.datetime.now().strftime("%Y-%m-%d %H-%M.csv")}')


def export_summarized_dataframe(params: dict, mini_df: pd.DataFrame):
    mini_df = mini_df.drop_duplicates(keep='first')
    mini_df.to_csv(
        f'./data/Fort {params["Subject"]} - miniDF - {datetime.datetime.now().strftime("%Y-%m-%d %H-%M.csv")}')
    ...
    for key, value in kwargs.items():
        if isinstance(value, pd.DataFrame):
            try:
                ...
                df.to_csv(
                    f'{folder}/Fort {params["Subject"]} Session {params["session"]} - {key} - {strftime("%Y-%m-%d %H-%M", localtime(params["startTime"]))}.csv')
            except:
                print("Something went wrong, keeping backup")
            else:
                backup_path = f'{folder}/Fort {params["Subject"]} Session {params["session"]} - {key} - {strftime("%Y-%m-%d %H-%M", localtime(params["startTime"]))}.backup.csv'
...
def save_backup(params: dict, **kwargs):
    ...
    for key, value in kwargs.items():
        if isinstance(value, pd.DataFrame):
            df = value.drop_duplicates(keep='first')
            df.to_csv(
                f'{folder}/Fort {params["Subject"]} Session {params["session"]} - {key} - {strftime("%Y-%m-%d %H-%M", localtime(params["startTime"]))}.backup.csv')
