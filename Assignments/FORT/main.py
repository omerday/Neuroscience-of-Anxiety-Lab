...
params = {
    ...
    'sequences': [None, None],  # Will be set to random NPU sequences below
...
# Generate random NPU sequences (n,p,u,n,p,u in random) - 2 cycles
def generate_random_npu_sequence():
    # Create 2 cycles of NPU (n,p,u,n,p,u)
    base_sequence = ['n', 'p', 'u', 'n', 'p', 'u']
    # Shuffle the sequence randomly
    random.shuffle(base_sequence)
    # Convert to string
    return ''.join(base_sequence).upper()

# Set the sequences to random NPU sequences
params['sequences'][0] = generate_random_npu_sequence()
params['sequences'][1] = generate_random_npu_sequence()
...
if params['blocks'] == 2:
    ...
    # Wait 5 minutes between sequences
