...
def wait_for_space_with_replay(window: visual.Window, io, params: dict, df: pd.DataFrame, dict_for_df: dict, timeout_seconds=None):
    keyboard = io.devices.keyboard
    keyboard.getKeys()
    core.wait(0.05)
    ...
    while True:
        ...
        if timeout_seconds is not None and (time.time() - start_time) >= timeout_seconds:
            return False, df  # Return False (not replay) when timeout reached
