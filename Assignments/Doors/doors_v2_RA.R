#####
# ProcessDoorsBehavioralData.R
# Created 12/13/20 by Sonia Ruiz
# This script processes behavioral data (joystick, VAS, questions) output from Doors

# Notes:
# 3/6/21 SR - added subject betas, betas plot by subj and dx, betas ANOVA
# 4/19/21 SR - added VAS and Post-Task Questions group analysis & plots
# 9/6/21 PN - defined dist_lm_group + added group lm summary table
# 4/20/22 SQ - debugged + updated to ggplot2
# 5/4/22 PN - added group-level distance beta analyses + plots (box & violin)
#####
...
#rootDir <- "C:/Users/newsomep2/Doors_process/subjects_v2"
rootDir <- "/Users/ranyabend/Documents/RU/studies/approach-avoid/doors/NIH_12.22/data/data files 10.4.23/raw2"
plotDir <- "/Users/ranyabend/Documents/RU/studies/approach-avoid/doors/NIH_12.22/data/data files 10.4.23/plots"
setwd(rootDir)

#any packages need to be updated? If so, update using "update.packages()"
#old.packages()

#update.packages(ask = FALSE, checkBuilt = TRUE)
# Set up subjects
#dx <- read.csv("L:/SDAN1/Data/conditioning/Doors/process/doors_dx_new.csv", header=T)

dx <- read.csv("/Users/ranyabend/Documents/RU/studies/approach-avoid/doors/NIH_12.22/data/data files 10.4.23/doors_dx_MAIN_2.csv", header=T)

subjList <- list.dirs(path=".", full.names = FALSE, recursive = FALSE)
exclude <- c("24477", "24593", "24611")
#exclude <- c("24270","24359", "24393", "24475", "24476", "24506", "24516", "24538", "24540", "24559", "eprime", "IRTA data", "test", "24630")
dim(dx)
head(dx)

#print(exclude)
subjList <- subset(subjList,!(subjList %in% exclude))
...
print(paste("length of subjList:" ,length(subjList)) )

# Import subject data
for (s in 1:length(subjList)) {
  subjDir <- paste(rootDir, subjList[s], sep="/") 
  print(subjDir)
  setwd(subjDir)
  ...
  df <- read.table(fileName[1], header=TRUE, fill=TRUE, sep = ",")
  
  df <- data.frame(df)
  
  # Select task sections
...
rm(fileName, s, dist_subj, q_subj, vas_subj)
...
# Add dx and other subject variables
...
dist_data$Sex <- as.numeric(as.factor(dist_data$Sex))
dist_data$Version <- as.numeric(as.factor(dist_data$Version))
...
group_vas <- group_vas %>%
  mutate(VAS_section = factor(VAS_section, levels=c(1,2,3,4),
                              labels = c("vas_pre", "vas_1", "vas_mid","vas_post")),
...
# Group average: STDEV ~ RP curve
ggplot(group_dist, aes(x=Punish, y=sd, 
                       group=factor(Reward), 
                       color=factor(Reward))) +
  geom_line(size=0.75) + 
  geom_point() +
  #geom_errorbar(size=0.75, width=0.2, aes(ymin = Distance-se, ymax = Distance+se)) +
  labs(x = "Punishment", y = "Distance StDev", 
       color = "Reward", 
       title = "Distance StDev ~ Reward x Punishment") +
  scale_x_continuous(breaks = c(0:10)) +
  theme_bw()

ggsave("distSTDEV_rp_group.png")

# Subj average: STDEV ~ RP curve 
ggplot(subj_dist, aes(x=Punish, y=sd, 
                      group = factor(Reward), 
                      color=factor(Reward))) +
  #geom_line() + 
  geom_point() +
  ...
  labs(x = "Punishment", y = "Distance StDev", 
       color = "Reward", 
       title = "Distance StDev ~ Reward x Punishment (subject)") +
  scale_x_continuous(breaks = c(0:10)) +
  facet_wrap(.~Subject) +
  theme_bw()

ggsave("distSTDEV_rp_by_subj.png")
