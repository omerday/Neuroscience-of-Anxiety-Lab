...
negative_df = df.copy()
negative_df = negative_df[negative_df["Biopac"] == 31]
negative_df["duration"] = 22.0
negative_df = negative_df[["Time", "duration"]].round(2)
negative_df.to_csv(f"subject-{subject_id}_run-{run}_negative-block_events-onset.txt", "\t", index=False)
...
neutral_df = df.copy()
neutral_df = neutral_df[neutral_df["Biopac"] == 51]
neutral_df["duration"] = 22.0
neutral_df = neutral_df[["Time", "duration"]].round(2)
neutral_df.to_csv(f"subject-{subject_id}_run-{run}_neutral-block_events-onset.txt", "\t", index=False)
...
positive_df = df.copy()
positive_df = positive_df[positive_df["Biopac"] == 71]
positive_df["duration"] = 22.0
positive_df = positive_df[["Time", "duration"]].round(2)
positive_df.to_csv(f"subject-{subject_id}_run-{run}_positive-block_events-onset.txt", "\t", index=False)
