...
def initiate_medoc_device():
    ...
    messagebox.addText(
        "Medoc device is initializing, please make sure the patient isn't connected to the thermode and press OK")
    messagebox.show()
...
def deliver_pain(window: visual.Window, temp, device, params: dict):
    ...
    print(current_time, " : 113 - heatHandler - ", f"{round(time.time() - start_time, 2)} - Raising to temp {temp}",
          " Ramp Time=", params['tempRampUpTime'], "\n")

    device.erase_conditional_events()
    tokens = []
    temperatures = []

    resp = device.finite_ramp_by_temperature(temp, 0.1, 0.1, is_stop_on_response_unit_yes=False,
                                             time=params['tempRampUpTime'])
    ...
    resp = device.finite_ramp_by_temperature(32, 0.1, 0.1, is_stop_on_response_unit_yes=False,
                                             time=params['tempRampUpTime'])
    ...
    now = datetime.now()
    current_time = now.strftime("%H:%M:%S.%f")
    print(current_time, " : 209 - heatHandler - Elapsed time since start: ",
          f"{round(time.time() - start_time, 2)} - till end", "\n")
